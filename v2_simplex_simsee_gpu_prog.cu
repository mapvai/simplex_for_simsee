...
int main(int argc, char** argv) {
	...
	if (argc < 3) {
        std::cout << "Usage: " << argv[0] << " filename trayectorias" << std::endl;
        return 1;
    }
...
void ini_mem(
	...
	for (int kTrayectoria = 0; kTrayectoria < NTrayectorias; kTrayectoria++) {
		...
		// pedir memoria en el dispositivo
		cudaMalloc(&h_simplex_array[kTrayectoria].x_inf, NVariables*sizeof(double));
		cudaMalloc(&h_simplex_array[kTrayectoria].x_sup, NVariables*sizeof(double));
		cudaMalloc(&h_simplex_array[kTrayectoria].flg_x, NVariables*sizeof(int8_t));
		cudaMalloc(&h_simplex_array[kTrayectoria].top, NVariables*sizeof(int));
		cudaMalloc(&h_simplex_array[kTrayectoria].flg_y, NRestricciones*sizeof(int8_t));
		cudaMalloc(&h_simplex_array[kTrayectoria].left, NRestricciones*sizeof(int));
		cudaMalloc(&h_simplex_array[kTrayectoria].mat, (NVariables + 1)*(NRestricciones + 1)*sizeof(double));
		
		// copiar la memoria al dispositivo
		...
		cudaMemcpy(h_simplex_array[kTrayectoria].flg_x, simplex_array[kTrayectoria].flg_x, NVariables*sizeof(int8_t), cudaMemcpyHostToDevice);
...
void read_csv_file(const char* filename, TSimplexGPUs& simplex) {
  ...
  for (int i = 0; i < simplex.NVariables; i++) {
    ...
    // read left
	simplex.left = new int[simplex.NRestricciones];
	getline(file, line, ';'); // jump first value
	for (int i = 0; i < simplex.NRestricciones; i++) {
		getline(file, line, ';');
		simplex.left[i] = stoi(line);
	}
	getline(file, line); // complete the consumption of the line
	...
	// read matrix
	simplex.mat = new double[(simplex.NRestricciones + 1) * (simplex.NVariables + 1)];
	for (int i = 0; i < simplex.NRestricciones + 1; i++) {
		getline(file, line, ';'); // jump first value
		for (int j = 0; j < simplex.NVariables + 1; j++) {
			getline(file, line, ';');
			simplex.mat[i*(simplex.NVariables + 1) + j] = stod(line);
		}
		getline(file, line); // complete the consumption of the line
	}
...
void clean_copy_simplex(TSimplexGPUs& simplex_from, TSimplexGPUs& simplex_to) {
  ...
  for (int i = 0; i < simplex_to.NRestricciones; i++) {
	...
	for (int i = 0; i < simplex_to.NRestricciones + 1; i++) {
		for (int j = 0; j < simplex_to.NVariables + 1; j++) {
			simplex_to.mat[i*(simplex_to.NVariables + 1) + j] = simplex_from.mat[i*(simplex_to.NVariables + 1) + j];
		}
	}
}
